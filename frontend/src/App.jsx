...
export default function TimetableApp() {
  ...
  const isLab = (code) =>
    code.includes("LAB") || code.startsWith("PMC");
  ...
  const getClassInfo = (day, time) => {
    for (const s of timetableData) {
      const t = s.timings?.find(
        (x) => x.day === day && x.time === time
      );
      if (t) {
        return {
          ...s,
        ...
        {showLeaveForm && (
          <div className="bg-white p-6 rounded-xl shadow mb-6">
            ...
            >
              ...
              <button
                className="col-span-full bg-indigo-600 text-white p-3 rounded-lg"
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-indigo-600 text-white">
                    <th className="p-4 sticky left-0 bg-indigo-600">
                      Day
                    </th>
                    {timeSlots.map((slot) => (
